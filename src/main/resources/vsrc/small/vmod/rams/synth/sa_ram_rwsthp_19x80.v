...
// ================================================================
// File Name: sa_ram_rwsthp_19x80.v
`timescale 1ns / 10ps
...
// This wrapper consists of : 1 Ram cells: AUTOSARAMDP_20X80_GL_M1_E2 ;
...
sa_ram_rwsthp_19x80_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rwsthp_19x80 (
...
integer row;
begin
// initializing AUTOSARAMDP_20X80_GL_M1_E2
for (row = 0; row < 19; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rwsthp_19x80.ram_Inst_19X80.mem_write(row - 0, shadow_mem[row][79:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [79:0] data;
begin
// probing AUTOSARAMDP_20X80_GL_M1_E2
 if (row >= 0 && row < 19) data[79:0] = r_sa_ram_rwsthp_19x80.ram_Inst_19X80.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
