...
// This wrapper consists of : 1 Ram cells: AUTOSARAMDP_16X272_GL_M1_E2 ;
...
sa_ram_rws_16x272_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rws_16x272 (
...
integer row;
begin
// initializing AUTOSARAMDP_16X272_GL_M1_E2
for (row = 0; row < 16; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rws_16x272.ram_Inst_16X272.mem_write(row - 0, shadow_mem[row][271:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [271:0] data;
begin
// probing AUTOSARAMDP_16X272_GL_M1_E2
 if (row >= 0 && row < 16) data[271:0] = r_sa_ram_rws_16x272.ram_Inst_16X272.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
