...
// This wrapper consists of : 1 Ram cells: AUTOSARAMPDP_64X116_GL_M1_D2 ;
...
sa_ram_rws_64x116_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rws_64x116 (
...
integer row;
begin
// initializing AUTOSARAMPDP_64X116_GL_M1_D2
for (row = 0; row < 64; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rws_64x116.ram_Inst_64X116.mem_write(row - 0, shadow_mem[row][115:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [115:0] data;
begin
// probing AUTOSARAMPDP_64X116_GL_M1_D2
 if (row >= 0 && row < 64) data[115:0] = r_sa_ram_rws_64x116.ram_Inst_64X116.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
