// sa_ram_rwsthp_80x18: synthesizable model wrapper
...
// This wrapper consists of : 1 Ram cells: AUTOSARAMDP_80X18_GL_M1_E2 ;
...
sa_ram_rwsthp_80x18_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rwsthp_80x18 (
...
integer row;
begin
// initializing AUTOSARAMDP_80X18_GL_M1_E2
for (row = 0; row < 80; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rwsthp_80x18.ram_Inst_80X18.mem_write(row - 0, shadow_mem[row][17:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [17:0] data;
begin
// probing AUTOSARAMDP_80X18_GL_M1_E2
 if (row >= 0 && row < 80) data[17:0] = r_sa_ram_rwsthp_80x18.ram_Inst_80X18.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
