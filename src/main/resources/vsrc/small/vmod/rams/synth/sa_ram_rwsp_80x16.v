// sa_ram_rwsp_80x16: synthesizable model wrapper
...
// This wrapper consists of : 1 Ram cells: AUTOSARAMPDP_80X16_GL_M2_D2 ;
...
sa_ram_rwsp_80x16_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rwsp_80x16 (
...
integer row;
begin
// initializing AUTOSARAMPDP_80X16_GL_M2_D2
for (row = 0; row < 80; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rwsp_80x16.ram_Inst_80X16.mem_write(row - 0, shadow_mem[row][15:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [15:0] data;
begin
// probing AUTOSARAMPDP_80X16_GL_M2_D2
 if (row >= 0 && row < 80) data[15:0] = r_sa_ram_rwsp_80x16.ram_Inst_80X16.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
