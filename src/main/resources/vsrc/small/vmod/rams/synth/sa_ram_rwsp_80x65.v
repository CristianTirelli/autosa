...
// This wrapper consists of : 1 Ram cells: AUTOSARAMPDP_80X66_GL_M1_D2 ;
...
sa_ram_rwsp_80x65_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rwsp_80x65 (
...
integer row;
begin
// initializing AUTOSARAMPDP_80X66_GL_M1_D2
for (row = 0; row < 80; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rwsp_80x65.ram_Inst_80X66.mem_write(row - 0, shadow_mem[row][64:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [64:0] data;
begin
// probing AUTOSARAMPDP_80X66_GL_M1_D2
 if (row >= 0 && row < 80) data[64:0] = r_sa_ram_rwsp_80x65.ram_Inst_80X66.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
