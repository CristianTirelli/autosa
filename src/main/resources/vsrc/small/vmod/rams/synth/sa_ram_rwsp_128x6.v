...
// This wrapper consists of : 1 Ram cells: AUTOSARAMDP_128X6_GL_M2_E2 ;
...
sa_ram_rwsp_128x6_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rwsp_128x6 (
...
integer row;
begin
// initializing AUTOSARAMDP_128X6_GL_M2_E2
for (row = 0; row < 128; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rwsp_128x6.ram_Inst_128X6.mem_write(row - 0, shadow_mem[row][5:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [5:0] data;
begin
// probing AUTOSARAMDP_128X6_GL_M2_E2
 if (row >= 0 && row < 128) data[5:0] = r_sa_ram_rwsp_128x6.ram_Inst_128X6.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
