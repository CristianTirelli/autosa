// sa_ram_rwsthp_60x21: synthesizable model wrapper
...
// This wrapper consists of : 1 Ram cells: AUTOSARAMDP_60X22_GL_M1_E2 ;
...
sa_ram_rwsthp_60x21_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rwsthp_60x21 (
...
integer row;
begin
// initializing AUTOSARAMDP_60X22_GL_M1_E2
for (row = 0; row < 60; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rwsthp_60x21.ram_Inst_60X22.mem_write(row - 0, shadow_mem[row][20:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [20:0] data;
begin
// probing AUTOSARAMDP_60X22_GL_M1_E2
 if (row >= 0 && row < 60) data[20:0] = r_sa_ram_rwsthp_60x21.ram_Inst_60X22.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
