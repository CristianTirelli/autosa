// sa_ram_rwsthp_60x168: synthesizable model wrapper
...
// This wrapper consists of : 1 Ram cells: AUTOSARAMPDP_60X168_GL_M1_D2 ;
...
sa_ram_rwsthp_60x168_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rwsthp_60x168 (
...
integer row;
begin
// initializing AUTOSARAMPDP_60X168_GL_M1_D2
for (row = 0; row < 60; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rwsthp_60x168.ram_Inst_60X168.mem_write(row - 0, shadow_mem[row][167:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [167:0] data;
begin
// probing AUTOSARAMPDP_60X168_GL_M1_D2
 if (row >= 0 && row < 60) data[167:0] = r_sa_ram_rwsthp_60x168.ram_Inst_60X168.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
