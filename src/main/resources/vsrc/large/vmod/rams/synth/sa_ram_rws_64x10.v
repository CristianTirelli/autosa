...
// This wrapper consists of : 1 Ram cells: AUTOSARAMDP_64X10_GL_M2_E2 ;
...
sa_ram_rws_64x10_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rws_64x10 (
...
integer row;
begin
// initializing AUTOSARAMDP_64X10_GL_M2_E2
for (row = 0; row < 64; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rws_64x10.ram_Inst_64X10.mem_write(row - 0, shadow_mem[row][9:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [9:0] data;
begin
// probing AUTOSARAMDP_64X10_GL_M2_E2
 if (row >= 0 && row < 64) data[9:0] = r_sa_ram_rws_64x10.ram_Inst_64X10.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
