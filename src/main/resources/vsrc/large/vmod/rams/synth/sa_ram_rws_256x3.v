...
// This wrapper consists of : 1 Ram cells: AUTOSARAMDP_256X4_GL_M2_E2 ;
...
sa_ram_rws_256x3_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rws_256x3 (
...
integer row;
begin
// initializing AUTOSARAMDP_256X4_GL_M2_E2
for (row = 0; row < 256; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rws_256x3.ram_Inst_256X4.mem_write(row - 0, shadow_mem[row][2:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [2:0] data;
begin
// probing AUTOSARAMDP_256X4_GL_M2_E2
 if (row >= 0 && row < 256) data[2:0] = r_sa_ram_rws_256x3.ram_Inst_256X4.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
