...
// This wrapper consists of : 1 Ram cells: AUTOSARAMPDP_256X64_GL_M2_D2 ;
...
sa_ram_rws_256x64_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rws_256x64 (
...
integer row;
begin
// initializing AUTOSARAMPDP_256X64_GL_M2_D2
for (row = 0; row < 256; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rws_256x64.ram_Inst_256X64.mem_write(row - 0, shadow_mem[row][63:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [63:0] data;
begin
// probing AUTOSARAMPDP_256X64_GL_M2_D2
 if (row >= 0 && row < 256) data[63:0] = r_sa_ram_rws_256x64.ram_Inst_256X64.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
