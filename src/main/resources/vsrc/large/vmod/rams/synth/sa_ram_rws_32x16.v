...
// This wrapper consists of : 1 Ram cells: AUTOSARAMDP_32X16_GL_M1_E2 ;
...
sa_ram_rws_32x16_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rws_32x16 (
...
integer row;
begin
// initializing AUTOSARAMDP_32X16_GL_M1_E2
for (row = 0; row < 32; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rws_32x16.ram_Inst_32X16.mem_write(row - 0, shadow_mem[row][15:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [15:0] data;
begin
// probing AUTOSARAMDP_32X16_GL_M1_E2
 if (row >= 0 && row < 32) data[15:0] = r_sa_ram_rws_32x16.ram_Inst_32X16.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
