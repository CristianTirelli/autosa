...
// This wrapper consists of : 1 Ram cells: AUTOSARAMDP_32X32_GL_M1_E2 ;
...
sa_ram_rwsp_32x32_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rwsp_32x32 (
...
integer row;
begin
// initializing AUTOSARAMDP_32X32_GL_M1_E2
for (row = 0; row < 32; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rwsp_32x32.ram_Inst_32X32.mem_write(row - 0, shadow_mem[row][31:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [31:0] data;
begin
// probing AUTOSARAMDP_32X32_GL_M1_E2
 if (row >= 0 && row < 32) data[31:0] = r_sa_ram_rwsp_32x32.ram_Inst_32X32.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
