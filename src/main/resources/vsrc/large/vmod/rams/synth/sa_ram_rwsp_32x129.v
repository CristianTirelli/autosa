// sa_ram_rwsp_32x129: synthesizable model wrapper
...
// This wrapper consists of : 1 Ram cells: AUTOSARAMPDP_32X130_GL_M1_D2 ;
...
sa_ram_rwsp_32x129_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rwsp_32x129 (
...
integer row;
begin
// initializing AUTOSARAMPDP_32X130_GL_M1_D2
for (row = 0; row < 32; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rwsp_32x129.ram_Inst_32X130.mem_write(row - 0, shadow_mem[row][128:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [128:0] data;
begin
// probing AUTOSARAMPDP_32X130_GL_M1_D2
 if (row >= 0 && row < 32) data[128:0] = r_sa_ram_rwsp_32x129.ram_Inst_32X130.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
