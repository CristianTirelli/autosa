// sa_ram_rwsthp_20x8: synthesizable model wrapper
...
// This wrapper consists of : 1 Ram cells: AUTOSARAMDP_20X8_GL_M1_E2 ;
...
sa_ram_rwsthp_20x8_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rwsthp_20x8 (
...
integer row;
begin
// initializing AUTOSARAMDP_20X8_GL_M1_E2
for (row = 0; row < 20; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rwsthp_20x8.ram_Inst_20X8.mem_write(row - 0, shadow_mem[row][7:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [7:0] data;
begin
// probing AUTOSARAMDP_20X8_GL_M1_E2
 if (row >= 0 && row < 20) data[7:0] = r_sa_ram_rwsthp_20x8.ram_Inst_20X8.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
