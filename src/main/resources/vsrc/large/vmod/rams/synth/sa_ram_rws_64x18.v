...
// This wrapper consists of : 1 Ram cells: AUTOSARAMDP_64X18_GL_M2_E2 ;
...
sa_ram_rws_64x18_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rws_64x18 (
...
integer row;
begin
// initializing AUTOSARAMDP_64X18_GL_M2_E2
for (row = 0; row < 64; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rws_64x18.ram_Inst_64X18.mem_write(row - 0, shadow_mem[row][17:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [17:0] data;
begin
// probing AUTOSARAMDP_64X18_GL_M2_E2
 if (row >= 0 && row < 64) data[17:0] = r_sa_ram_rws_64x18.ram_Inst_64X18.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
