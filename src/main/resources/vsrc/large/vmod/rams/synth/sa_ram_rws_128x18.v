...
// This wrapper consists of : 1 Ram cells: AUTOSARAMPDP_128X18_GL_M2_D2 ;
...
sa_ram_rws_128x18_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_sa_ram_rws_128x18 (
...
integer row;
begin
// initializing AUTOSARAMPDP_128X18_GL_M2_D2
for (row = 0; row < 128; row = row + 1)
 if (shadow_written[row]) r_sa_ram_rws_128x18.ram_Inst_128X18.mem_write(row - 0, shadow_mem[row][17:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [17:0] data;
begin
// probing AUTOSARAMPDP_128X18_GL_M2_D2
 if (row >= 0 && row < 128) data[17:0] = r_sa_ram_rws_128x18.ram_Inst_128X18.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
